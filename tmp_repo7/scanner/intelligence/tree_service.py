...
class TreeService:
    ...
    def get_structure_view(self, graph: KnowledgeGraph) -> Dict[str, Any]:
        ...
        roots = self._build_recursive_tree(
            graph, 
            repo_node.id, 
            ["CONTAINS", "GENERATES_FINDING", "EXECUTES"],
            allowed_types={"Repository", "Folder", "File", "Class", "Function", "Finding"}
        )
    ...
    def get_directory_view(self, graph: KnowledgeGraph) -> Dict[str, Any]:
        ...
        def collect_file_findings(file_node_id: str) -> list:
            """Walk CONTAINS then GENERATES_FINDING from a File to gather its Findings."""
            findings = []
            visited = set()
            stack = [file_node_id]
            while stack:
                ...
                for edge in graph.get_out_edges(nid):
                    ...
                    elif edge.type == "CONTAINS" and child.type not in DIRECTORY_TYPES:
