    ...
    def build(self, parsed_files: list, findings: list) -> KnowledgeGraph:
        ...
        for pf in parsed_files:
            ...
            for func in pf.functions:
                ...
                if "decorator_method" in func.context:
                    for i, method in enumerate(func.context["decorator_method"]):
                        if any(m in method.lower() for m in [".get", ".post", ".put", ".delete", ".patch"]):
                            is_api = True
                            http_method = method.split(".")[-1].upper()
                            if "decorator_route" in func.context and i < len(func.context["decorator_route"]):
                                route = func.context["decorator_route"][i].strip('"\'')
                            break

                self.graph.add_node(GraphNode(
                    id=func.symbol_id, type="Function", properties={
                        "name": func_name, 
                        "snippet": func.name, 
                        "line": func.line,
                        "file": pf.file_path,
