...
- Model selector (all Ollama models currently pulled)
...
def render_chat_page(
    ...
    st.caption("Ask anything about your codebase, findings, or security policy — powered by local AI (Ollama + RAG).")
    ...
    with st.sidebar:
        ...
        model_input = st.text_input(
            "Ollama chat model",
            value=st.session_state.get("acg_model", "llama3.1"),
            help="Any model you have pulled in Ollama, e.g. llama3.1, qwen2.5, mistral",
            key="acg_model_input",
        )
    ...
    # ── Ollama health check ───────────────────────────────────────────
    if not copilot.is_ollama_healthy():
        st.error(
            "⚠️ **Ollama is not running or the model is not pulled.**\n\n"
            f"1. Start Ollama: `ollama serve`\n"
            f"2. Pull the model: `ollama pull {chat_model}`\n"
            "3. Refresh this page."
        )
        _render_ollama_setup_guide(chat_model)
        return
...
# ---------------------------------------------------------------------------
# Ollama setup guide (shown when Ollama is not running)
# ---------------------------------------------------------------------------

def _render_ollama_setup_guide(model: str) -> None:
    with st.expander("📖 How to set up Ollama", expanded=True):
        st.markdown(f"""
### Quick Setup Guide
...
Once Ollama is running and the model is pulled, the AI Copilot will activate automatically.

> 💡 **Tip:** Use `qwen2.5:7b` for faster responses on CPU, `llama3.1:70b` for highest quality on GPU.
        """)
