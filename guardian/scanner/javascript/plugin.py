...
_PATTERNS = [
    ...
    ("XSS", "Medium", re.compile(r'\.innerHTML\s*=\s*[^;]*\+')),
    ("Eval Injection", "High", re.compile(r'\beval\s*\(\s*\w')),
            ...
            for category, severity, pat in _PATTERNS:
                if pat.search(line):
                    findings.append(Finding(
                        ...
                        file=file_label, line=lineno, snippet=line.strip()[:200],
                        recommendation=f"Mitigate {category} using secure coding practices.",
                        confidence=0.6,
                    ))
                    break
