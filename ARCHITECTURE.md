...
├── llm/                    LLM LAYER — the only package that talks to a
│   ├── base.py             provider API. BaseLLM contract + error family
│   ├── config.py           env-driven LLMConfig (key never logged)
│   ├── nemotron.py         NVIDIA Nemotron (OpenAI-compatible + REST fallback)
│   ├── factory.py          create_llm() — Factory Pattern, DI entry point
│   ├── parser.py           Nemotron text -> SecurityAnalysis JSON (4-stage repair)
│   ├── guardrails.py       inbound injection/secret redaction; outbound
│   │                       hallucination/evidence/scope checks
│   └── prompt_builder.py   security-analysis prompt + context compression
├── ai/                     RAG + CodeBERT assistant (off by default)
│   ├── local_embedder.py   LOCAL sentence-transformers embeddings
│   ├── validator.py        mechanical hallucination detection
│   └── scan_context.py     findings-as-context + exact-match injection
...
3. **Deterministic core, AI enrichment.** The scan pipeline is fully
   functional offline with no API key. The Nemotron/RAG layer attaches on
   top for explanation, false-positive triage, and chat — it never gates
   results. `python -m guardian scan` works with NVIDIA_API_KEY unset.

6. **Split inference from embeddings.** Nemotron handles hosted chat
   inference. Embeddings stay local (`ai/local_embedder.py`) because
   FAISS indexes are dimension-locked and because embedding a whole
   repository through a remote API would transmit the entire codebase.

7. **Cloud LLM = new threat surface.** Nemotron is remote, so outbound
   secret redaction is mandatory (`llm/guardrails.py`) and context
   compression is a security control, not just a cost control. Air-gapped
   deployment is no longer available while Nemotron is the only registered
   provider — add a local provider to `llm/factory.py` to restore it.
...
- `intent-roadmap`  — optional Nemotron re-ranking of domain candidates
