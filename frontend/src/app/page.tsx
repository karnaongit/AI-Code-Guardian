...
import PRReviewPage from "../components/pr/PRReviewPage";
...
function NoScanPlaceholder({ title, onNavigate }: { title: string; onNavigate: () => void }) {
  return (
    <div className="space-y-5">
      <SectionHead title={title} />
      <div className="p-12 text-center bg-[#12131a] border border-white/8 rounded-xl space-y-4">
        <ShieldCheck className="w-12 h-12 mx-auto text-[#ff5400]/40" />
        <h3 className="text-sm font-mono font-bold text-[#f4f4f8] uppercase tracking-wider">NO ACTIVE SCAN REPORT</h3>
        <p className="text-xs font-mono text-[#8e8e9a] max-w-md mx-auto">
          Please select a local directory, upload a ZIP archive, or enter a GitHub repository URL in the <span className="text-[#ff5400] font-semibold">IDE Workspace</span> tab to initiate analysis.
        </p>
        <button
          onClick={onNavigate}
          className="glass-button px-4 py-2 text-xs font-mono font-bold text-white rounded-lg inline-flex items-center gap-2"
        >
          <Play className="w-3.5 h-3.5 text-[#ff5400]" />
          OPEN IDE WORKSPACE & RUN SCAN
        </button>
      </div>
    </div>
  );
...
function AppInner() {
  ...
  /* Ensure pre-scan mode starts clean on initial mount unless active scan is in progress */
  useEffect(() => {
    try {
      const activeScanId = sessionStorage.getItem("guardian_scan_id");
      const savedReport = sessionStorage.getItem("guardian_report");
      if (activeScanId && savedReport) {
  ...
  const handleUpdateFindings = useCallback((updatedFindings: any[]) => {
    setReport((prev: any) => {
      if (!prev) return null;
      const updated = {
        ...prev,
      ...
      try {
        sessionStorage.setItem("guardian_report", JSON.stringify(updated));
      } catch {}
      return updated;
    });
  }, []);
  ...
  /* ── Derived data (Dynamic based on current scan findings) ── */
  ...
  // Balanced, Proportional Security Score (100 = pristine, weighted penalty per flaw)
  const weightedPenalty = (critical * 18) + (high * 9) + (medium * 4) + (low * 1);
  const securityScore = totalVulnerabilities === 0 
    ? 100 
    : Math.max(10, Math.round(100 * Math.exp(-weightedPenalty / 80)));

  // Overall Risk Score (Inverse of Security Score)
  const overallRiskScore = totalVulnerabilities === 0 ? 0 : 100 - securityScore;

  // Quantum Cryptographic Readiness Score
  const cryptoFindings = findings.filter((f) => {
    const cwe = (f.cwe || f.cwe_id || "").toUpperCase();
    const cat = (f.category || f.title || "").toLowerCase();
    return cwe === "CWE-327" || cwe === "CWE-330" || cat.includes("crypto") || cat.includes("md5") || cat.includes("sha1");
  }).length;
  const quantumScore = totalVulnerabilities === 0 ? 100 : Math.max(15, 100 - (cryptoFindings * 25));

  // Dependency Vulnerability Health Score
  const depVulns = report?.dependencies?.vulnerable_count || 0;
  const depScore = Math.max(15, 100 - (depVulns * 20));

  // Business Policy Alignment Score
  const policyPenalty = (critical * 12) + (high * 6) + (medium * 2);
  const alignmentScore = totalVulnerabilities === 0 
    ? 100 
    : Math.max(15, Math.round(100 - policyPenalty));
      ...
      <div className="flex h-screen overflow-hidden text-[#f4f4f8] relative z-10">
        ...
        <main className="flex-1 overflow-y-auto bg-[#0B0F19]">
          ...
          <header className="sticky top-0 z-10 bg-[#0B0F19]/95 backdrop-blur-sm border-b border-white/7 px-6 py-3 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              {/* Scan stats pills — only shown after scan is completed */}
              {report && (
          ...
          <div className="px-5 py-3 space-y-3">
            ...
            {/* Triage funnel — shown only after scan is completed */}
            {report && <TriageFunnel metrics={metrics} />}

            {/* ── Tab content with fade-in transition ─ */}
            <PageTransition tabKey={activeTab}>

              {/* IDE Workspace — persistent mount so input URL, scan results, and AI reasoning box never reset */}
              <div className={activeTab === "workspace" ? "block" : "hidden"}>
                <IDEWorkspace onScanComplete={handleScanComplete} onUpdateFindings={handleUpdateFindings} />
              </div>

              {/* Mind Map */}
              {activeTab === "mindmap" && (
                !report ? (
                  <NoScanPlaceholder title="Code Mind Map & AST Topology" onNavigate={() => navigateTo("workspace")} />
                ) : (
              ...
              {activeTab === "overview" && (
                !report ? (
                  <NoScanPlaceholder title="Security Overview" onNavigate={() => navigateTo("workspace")} />
                ) : (
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                      {[
                        ...
                        { label:"ALIGNMENT",       value:alignmentScore, suffix:"/100", color: alignmentScore>=80?"text-emerald-400":alignmentScore>=50?"text-[#ff5400]":"text-red-400" },
                        { label:"QUANTUM READY",   value:quantumScore,   suffix:"/100", color: quantumScore>=80?"text-emerald-400":"text-amber-400" },
                        { label:"DEPENDENCIES",    value:depScore,       suffix:"/100", color: depScore>=80?"text-emerald-400":"text-amber-400" },
              ...
              {activeTab === "security_compliance" && (
                !report ? (
                  <NoScanPlaceholder title="Security Vulnerabilities" onNavigate={() => navigateTo("workspace")} />
                ) : (
                  <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                      {[
                        ...
                        { title:"CRYPTOGRAPHIC BILL OF MATERIALS", content:<div className="overflow-x-auto rounded bg-[#0c0d11] border border-white/8"><table className="w-full text-[9px] font-mono"><thead className="border-b border-white/8 text-[#8e8e9a]"><tr><th className="p-2">Algo</th><th className="p-2">Status</th><th className="p-2">Count</th><th className="p-2">Target</th></tr></thead><tbody><tr><td className="p-2 font-bold text-[#f4f4f8]">MD5</td><td className="p-2 text-[#ff5400]">Broken</td><td className="p-2">{findings.filter(f => (f.cwe||"").includes("327") || (f.category||"").toLowerCase().includes("crypto")).length}</td><td className="p-2 text-emerald-400">SHA-256</td></tr></tbody></table></div> },
              ...
              {activeTab === "pr_review" && (
                !report ? (
                  <NoScanPlaceholder title="Pull Request Automated Security Review" onNavigate={() => navigateTo("workspace")} />
                ) : (
                  <PRReviewPage
                    report={report}
                    onUpdateFindings={handleUpdateFindings}
                    onNavigateWorkspace={() => navigateTo("workspace")}
                  />
                )
              )}
              ...
              {activeTab === "business_intent" && (
                !report ? (
                  <NoScanPlaceholder title="Business Intent & Policy Engine" onNavigate={() => navigateTo("workspace")} />
                ) : (
              ...
              {activeTab === "reports" && (
                !report ? (
                  <NoScanPlaceholder title="Export Scan Reports" onNavigate={() => navigateTo("workspace")} />
                ) : (
                  <div className="space-y-5">
